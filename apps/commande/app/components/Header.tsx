      ...
      <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-6">
          ...
          {prenom ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleSignOut} className="font-body text-xs text-chocolat/30 hover:text-chocolat/60 transition-colors">
                Déconnexion
              </button>
            </div>
          ) : (
        ...
        {count > 0 && (
          <Link href="/panier"
            ...
            Panier
            <span className="w-5 h-5 bg-rose rounded-full flex items-center justify-center font-body text-xs text-white font-medium">
              {count > 9 ? '9+' : count}
            </span>
          </Link>
        )}
