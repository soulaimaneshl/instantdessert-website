    ...
    <>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-16">
        ...
        <nav className="font-body text-sm text-chocolat/40 flex items-center gap-2">
          ...
          <span>/</span>
          <Link href="/catalogue" className="hover:text-chocolat transition-colors">Délices</Link>
          <span>/</span>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6 snap-x snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {pairings.map(p => (
                <Link key={p.id} href={`/catalogue/${p.id}`}
                  className="group shrink-0 w-[260px] snap-start bg-white border border-blush rounded-2xl overflow-hidden hover:shadow-md hover:border-rose/30 transition-all flex flex-col">
                  <div className="h-44 bg-blush/20 flex items-center justify-center group-hover:bg-blush/30 transition-colors">
              ...
              {/* Spacer pour voir la dernière card entière */}
